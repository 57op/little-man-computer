...
static void lmc_placeholder_instruction(LitteManComputerContext *ctx, uint8_t data) {}

static void lmc_halt_instruction(LitteManComputerContext *ctx, uint8_t data) {
  ctx->executing = false;
}

static void lmc_add_instruction(LitteManComputerContext *ctx, uint8_t mailbox) {
...
static void lmc_subtract_instruction(LitteManComputerContext *ctx, uint8_t mailbox) {
...
static void lmc_store_instruction(LitteManComputerContext *ctx, uint8_t mailbox) {
  ctx->mailboxes[mailbox] = ctx->accumulator;
}

static void lmc_load_instruction(LitteManComputerContext *ctx, uint8_t mailbox) {
  ctx->accumulator = ctx->mailboxes[mailbox];
}

static void lmc_branch_instruction(LitteManComputerContext *ctx, uint8_t address) {
  ctx->program_counter = address;
}

static void lmc_branch_if_zero_instruction(LitteManComputerContext *ctx, uint8_t address) {
  if (ctx->accumulator == 0 && (ctx->flags & LMC_NEGATIVE_FLAG) == 0) {
    ctx->program_counter = address;
  }
}

static void lmc_branch_if_positive_instruction(LitteManComputerContext *ctx, uint8_t address) {
...
static void lmc_input_output_instruction(LitteManComputerContext *ctx, uint8_t op) {
...
static void (*lmc_instructions[])(LitteManComputerContext *ctx, uint8_t data) = {
